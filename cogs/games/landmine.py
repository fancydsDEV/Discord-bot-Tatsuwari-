...
class LandmineFrontend(disnake.ui.Button["Mine"]):
...
class LandmineBackend(disnake.ui.View):
    children: List[LandmineFrontend]
    player_one = 1
...
class RunLandmine(commands.Cog):
    ...
    async def landmine(self, inter):
        embed = disnake.Embed(title="**LETS GO, Dodge the bombs,...HHU**💣",
                              ...
                              colour=0x5ba082)
        await inter.send(embed=embed, view=LandmineBackend(), delete_after=400)


def setup(bot: commands.bot):
    bot.add_cog(RunLandmine(bot))
