...
class AniInfo(commands.Cog):
    ...
    async def anime(self, inter: disnake.ApplicationCommandInteraction, anime_name: str):
        """Get info about anime"""
        await inter.response.defer()
        try:
            querystring = {f"page": "1", "size": "10", "search": f"{anime_name}"}
            ...
            embed = disnake.Embed(title='Anime Info',
                                  description=f'Information About {anime_name}',
                                  colour=0x8E3941,
            ...
            await inter.send(f"**Anime Not Found** *~404~*~**{anime_name}**~", delete_after=50)


def setup(bot: commands.bot):
    bot.add_cog(AniInfo(bot))
